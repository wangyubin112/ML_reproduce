...
# wyb
def convert_throttle_to_carla(throttle, min_throttle, max_throttle):
    """
    ...
    return (1 - t) * min_throttle + max_throttle * t

def clip_action_diff(action, action_prev, space_min, space_max):
    """
    :param steering: (float)
    :return: (foat)
    """
    max_diff = (0.15 - 1e-5) * 2 # (MAX_STEERING_DIFF - 1e-5) * (MAX_STEERING - MIN_STEERING)
    diff = np.clip(action - action_prev, -max_diff, max_diff)
    return np.clip(action_prev + diff, space_min, space_max)
