...
from carla_gym.carla_lap_env import CarlaLapEnv as CarlaEnv
from utils import common_carla
...
def load_vae(path=None, z_size=None, input_dimension=(80, 160, 3)):
    """
    :param path: (str)
    :param z_size: (int)
    ...
    vae = VAEController(z_size=z_size, input_dimension=input_dimension)
    if path is not None:
        vae.load(path)
...
def make_env(level=0, seed=0, log_dir=None, vae=None, frame_skip=None,
             teleop=False, n_stack=1, obs_res=(160, 80)):
    """
    ...
    if log_dir is None and log_dir != '':
        log_dir = "logs/reward/{}/".format(int(time.time())) # wyb
    ...
    def _init():
        ...
        encode_state_fn = common_carla.create_encode_state_fn(vae, measurements_to_include) # vae encode
        reward_fn = common_carla.reward_fn
        env = CarlaEnv(obs_res=obs_res, action_smoothing=0,
                   encode_state_fn=encode_state_fn, reward_fn=reward_fn,
                   synchronous=True, fps=FPS, host=HOST) # wyb '10.38.164.121' '127.0.0.1'
        env.seed(0) # wyb
