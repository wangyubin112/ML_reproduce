...
class VAEController:
    ...
    def encode(self, observation):
        ## wyb: use vae_train in Linux and train in Win
        assert observation[0].shape == self.input_dimension, "{} != {}".format(observation.shape, self.input_dimension)
        # Normalize
        observation = preprocess_input(observation[0].astype(np.float32),
    ...
    def load(self, path):
        self.target_vae = ConvVAE.load(path, self.input_dimension) # wyb
