...
class ConvVAE(object):
    ...
    def __init__(self, z_size=32, batch_size=100, learning_rate=0.0001,
                 kl_tolerance=0.5, is_training=True, beta=1.0, reuse=False, shape_in=(80, 160, 3)):
        self.z_size = z_size
    ...
    def _build_graph(self):
        self.graph = tf.Graph()
        with self.graph.as_default():
            self.input_tensor = tf.placeholder(tf.float32, shape=[None, self.shape_in[0], self.shape_in[1], self.shape_in[2]])
            self.target_tensor = tf.placeholder(tf.float32, shape=[None, self.shape_in[0], self.shape_in[1], self.shape_in[2]])
